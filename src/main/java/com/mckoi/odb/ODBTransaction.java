...
//  static final Key RESOURCE_PROPERTIES_KEY = new Key((short) 0, 1, 31);
   ...
   * Constructor.
   */
  ODBTransaction(ODBSession session,
...
//  /**
//   * Defines a class and returns an ODBClass for it. After a class is defined
//   * objects may be created in the database with this class specification. All
//   * classes defined are automatically added to a named list called
//   * 'SYS.Classes'. The system uses this when looking up class specification.
//   * If a class name isn't found in the list when a new object with the type
//   * is created then an error is generated.
//   * <p>
//   * If two classes are created with the same name then an exception is
//   * generated or a fault generated at commit.
//   */
//  private ODBClass defineClass(ODBClassDefinition class_def) {
//
//    // The class name,
...
//    ODBObject class_obj =
//            constructObject(ODBClasses.CLASS, class_name, class_serialization);
//    Reference class_ref = class_obj.getReference();
//
//    try {
//
...
//
//    // Done.
//    return new MaterializedODBClass(class_ref, class_name, class_serialization);
//  }
  ...
  ODBObject constructObject(ODBClass clazz, Reference obj_ref, Object... args) {
    ...
    try {
      ...
      // Return the object
      return new MaterializedODBObject(clazz, obj_ref, content_copy);
  ...
  public ODBClass getClass(Reference ref) {
    ...
    if (odb_class == null) {
      ...
      odb_class = new MaterializedODBClass(ref, class_name, serialization);
...
//          }
      ...
      ob = new MaterializedODBObject(type, ref, content);
      return ob;
...
//  /**
//   * Returns a ODBData object given a reference. Returns null if the reference
//   * is to a null Data. The returned object is mutable and changes made are
//   * published on commit.
//   * <p>
//   * Throws NoSuchReferenceException if the reference is invalid.
//   */
//  public ODBData getData(Reference ref) {
...
//  }
  ...
  public List<String> getNamedItemsList() {
    return new FirstFieldObjectList(ODBClasses.NAMER, getNamedItemList());
  }
  ...
  public List<String> getClassNamesList() {
    return new FirstFieldObjectList(ODBClasses.CLASS, getSystemClassList());
  }
...
//    System.out.println();
   ...
   */
  void copyDictionaryAdditionTo(ODBTransaction dst_transaction,
                                DictionaryEvent evt) {
   ...
   * new key.
   */
  Key copyResourceAsNewKeyTo(ODBTransaction destination,
   ...
   * key.
   */
  Key copyResourceTo(ODBTransaction destination,
                     Reference resource_ref, Key resource_key) {
   ...
   * transaction.
   */
  void copyListTo(ODBTransaction destination, Reference list_reference,
   ...
   * as appropriate.
   */
  void replayObjectChange(ODBTransaction from_transaction,
   ...
   * as appropriate.
   */
  void replayDataChange(ODBTransaction from_transaction,
                        DataChangeEvent evt) {
...
//    String unique_class_name;
//    String class_name = clazz.getName();
//    if (class_name.startsWith("$")) {
//      if (class_name.equals("$List")) {
//        // All list references are stored in the same bucket,
//        unique_class_name = "$List";
//      }
//      else if (class_name.equals("$String")) {
//        unique_class_name = "$String";
//      }
//      else if (class_name.equals("$Data")) {
//        unique_class_name = "$Data";
//      }
//      else {
//        throw new RuntimeException(MessageFormat.format(
//                          "Unrecognized system class type: {0}", class_name));
//      }
//    }
//    else {
//      unique_class_name = clazz.getInstanceName();
//    }
//
//    PropertySet pset = new PropertySet(
//                         transaction.getDataFile(BUCKET_PROPERTIES_KEY, 'w'));
//    // Get the bucket from the property map
//    long bucket_id = pset.getLongProperty(unique_class_name, -1);
//    // If there is no bucket id then create a new one from a global sequence,
//    if (bucket_id == -1) {
//      bucket_id = pset.getLongProperty("!id_count", 100);
//      pset.setLongProperty("!id_count", bucket_id + 1);
//      pset.setLongProperty(unique_class_name, bucket_id);
//    }
//
//    // The repository for the class data,
//    Key bucket_key = new Key((short) 0, 2, bucket_id);
//
//    // Turn it into an OrderedSetData object,
//    return new OrderedSetData(
//                 transaction.getDataFile(bucket_key, 'w'), object_comparator);
  ...
  private ByteArray createObjectSer(Reference ref,
    ...
    IntByteArrayStream bout = new IntByteArrayStream(512);
    DataOutputStream dout = new DataOutputStream(bout);
    int field_count = clazz.getFieldCount();
...
//  /**
//   * Allocates a DataFile resource against the given reference and returns
//   * a key string for accessing the resource.
//   */
//  private ResourceKey allocateResource(Reference ref, int field_num) {
//
//    String ref_string = ref.toString() + "." + field_num;
//
//    PropertySet pset = new PropertySet(
//                        transaction.getDataFile(RESOURCE_PROPERTIES_KEY, 'w'));
//    // Get the bucket it from the property map
//    long bucket_id = pset.getLongProperty(ref_string, -1);
//
//    // If there is no bucket id then create a new one from a global sequence,
//    if (bucket_id == -1) {
//      bucket_id = pset.getLongProperty("!id_count", 100);
//      pset.setLongProperty("!id_count", bucket_id + 1);
//      pset.setLongProperty(ref_string, bucket_id);
//    }
//
////    System.out.println("ref_string = " + ref_string);
////    System.out.println("bucket_id = " + bucket_id);
////    System.out.println(pset);
//
//    // Return the bucket_id value for this resource,
//    return new ResourceKey(0, bucket_id);
//
//  }

//  /**
//   * Writes an object with the given class to the backed stores and returns
//   * a reference to it. The given ODBClass object is class of the object
//   * being written.
//   * <p>
//   * This has a side effect of modifying the 'args' array with the
//   * materialized objects for the object.
//   */
//  private Reference writeToDB(ODBClass clazz, Object[] args) {
//
////    System.out.println("Writing object of type; " + clazz.toString());
//
//
//  }
...
//    try {
//      IntByteArrayStream bout = new IntByteArrayStream(16);
//      DataOutputStream dout = new DataOutputStream(bout);
//
//      // Write the unique reference id,
//      dout.writeLong(ref.getHighLong());
...
//    }
  ...
  private ODBData createODBData(Reference data_reference) {
    ...
    // The data object,
    return new Data(ODBTransaction.this, data_reference, data);
...
//  /**
//   * Sets an inline string on an object stored in the database. An exception
//   * is generated if the field is not mutable or the object is not found.
//   */
//  private void internalSetObjectString(ODBClass clazz, Reference ref,
...
//      ByteArray search_key = createSearchKey(ref);
//      // The tail set from the search key
//      OrderedSetData found_set = bucket_set.tailSet(search_key);
//      // If the tail set is empty or the found entry doesn't match, generate
//      // an error.
//      if (found_set.isEmpty()) {
//        throw new RuntimeException(
...
//      }
//
//      ByteArray ob_ser = found_set.first();
//      DataInputStream din = ob_ser.getDataInputStream();
//
//      // Check the ref of the found record matches,
//      long rh = din.readLong();
//      long rl = din.readLong();
...
//      System.out.println("Key = " + key);
    ...
    public ODBObject getObject(int i) {
      ...
      return ODBTransaction.this.getObject(field_class, ref);
    }
...
//  static class ResourceKey extends Integer128Bit {
//
//    ResourceKey(long high, long low) {
//      super(high, low);
//    }
//
//    ResourceKey(long[] ref) {
//      super(ref);
//    }
//
//  }
