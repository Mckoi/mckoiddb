...
public abstract class AbstractStore implements Store {
  ...
  private boolean repairScan(final ArrayList areas_to_fix,
    ...
    // We guarantee to be scan forward if we get here....
   ...
   * function is useful for determining if any areas have leaked (not been
   * deleted and not part of the graph).
   */
  public synchronized void sortedAreaScan(Iterator64Bit sorted_area_set)
                                                          throws IOException {
    final long end_of_data_area = endOfDataAreaPointer();
    long[] header = new long[2];
    // The first header
    ...
    long last_area_p = Long.MAX_VALUE;
    if (sorted_area_set.hasNext()) {
      last_area_p = sorted_area_set.next();
    }

    while (pointer < end_of_data_area) {
      getAreaHeader(pointer, header);
      long area_size = (header[0] & 0x07FFFFFFFFFFFFFFFL);
      // If the area is not deleted
      if ((header[0] & 0x08000000000000000L) == 0) {
        // These are all areas deleted in store but marked in the graph
        while (last_area_p < pointer) {
          System.out.println("Area deleted in store: " + last_area_p);
          if (sorted_area_set.hasNext()) {
            last_area_p = sorted_area_set.next();
          }
          else {
            last_area_p = Long.MAX_VALUE;
          }
        }
        // If pointer is the same as last pointer from set
        if (last_area_p == pointer) {
          // Go to next pointer
          if (sorted_area_set.hasNext()) {
            last_area_p = sorted_area_set.next();
          }
        ...
        // If pointer is less than the last pointer from set,
        else if (last_area_p > pointer) {
          System.out.println("Area not in graph: " + pointer);
//          deleteArea(pointer);
        }
      }
      pointer += area_size;
   ...
   */
  public LongList getAllAreas() throws IOException {
...
//  }
  ...
  /**
   * Returns the total allocated space since the file was opened.
   */
...
//    System.out.println("- Removing from index " + bin_chain_index);
  ...
  private void cropArea(long pointer, long allocated_size) throws IOException {
    ...
    // If the difference is greater than 32 bytes, add the excess space to
    // a free bin.
    boolean is_wilderness = (pointer == wilderness_pointer);
    if ((is_wilderness && size_difference >= 32) || size_difference >= 32) {
  ...
  private long alloc(long size) throws IOException {
    ...
    for (int i = bin_chain_index;
      ...
      if (cur_pointer != -1) {
        ...
        // We only search the first 16 entries in the bin before giving up.
        else {
          long last_pointer = -1;
          int searches = 0;
          while (cur_pointer != -1 &&
    ...
    if (found_bin_index == -1) {
      ...
      if (( working_pointer & 0x0F000000000000000L ) != 0) {
      ...
      // Check the pointer is within the first 60 bits
      if (( free_area_pointer & 0x0F000000000000000L ) != 0) {
        throw new IOException("Address space exhaustion.");
      }
