/**
 * com.mckoi.store.HeapStore  20 Feb 2003
 *
...
/**
 * An implementation of the Store interface that stores information in 
...
public final class HeapStore implements Store {
  ...
  public AreaWriter createArea(long size) throws IOException {
    // Heap store doesn't support areas larger than Integer.MAX_VALUE
    if (size < 0) {
    ...
    synchronized (this) {
      // If unique_id_key is greater than 60bits the address space is exhausted
      if ((unique_id_key & 0x0F000000000000000L) != 0) {
