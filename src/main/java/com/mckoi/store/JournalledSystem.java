...
class JournalledSystem {
  ...
  void start() throws IOException {
    ...
    lock_file = new FileOutputStream(new File(journal_path, "lock.m"));
    boolean cant_lock = false;
    try {
    ...
    if(cant_lock) {
      throw new IOException(
              "Can not start because the 'lock.m' file is locked. " +
              "This usually is caused when multiple JVM instances attempt " +
              "to access the same data store in the file system.");
    }
  ...
  void stop() throws IOException {
    ...
    synchronized (all_resources) {
      Iterator i = all_resources.keySet().iterator();
      while (i.hasNext()) {
        String resource_name = (String) i.next();
        AbstractResource r = getResource(resource_name);
        r.persistClose();
      }
    }
    ...
    file_lock.release();
    lock_file.close();
    new File(journal_path, "lock.m").delete();
...
//    }
  ...
  public JournalledResource createResource(String resource_name) {
    ...
    synchronized (all_resources) {
      ...
      if (resource == null) {
        ...
        if (ENABLE_LOGGING) {
          resource = new Resource(resource_name, id, accessor, page_size);
        }
        else {
          resource = new NonLoggingResource(resource_name, id, accessor, page_size);
        }
        // Put this in the map.
   ...
   * method returns we are guaranteed that all the journals are flushed and the
...
//        final long absolute_position = page_number * page_size;
        ...
        // Write the header
        long resource_id = v.longValue();
        data_out.writeLong(21);
        data_out.writeInt(8 + 8 + 4 + 4 + 4 + len);
        data_out.writeLong(resource_id);
        data_out.writeLong(page_number);
        data_out.writeInt(page_size);
...
//        data_out.writeInt(off + (int) (absolute_position & 8191));
     ...
     */
    void buildPage(long in_page_number, int in_page_size,
      ...
      synchronized (this) {
        data.readFully(position, buffer, 0, 8);
        type = ByteArrayUtil.getLong(buffer, 0);
        if (type == 1) {
          data.readFully(position + 8, buffer, 0, 28);
          resource_id = ByteArrayUtil.getLong(buffer, 4);
          page_number = ByteArrayUtil.getLong(buffer, 12);
          page_size   = 8192;
          page_offset = ByteArrayUtil.getInt(buffer, 20);
          page_length = ByteArrayUtil.getInt(buffer, 24);
          data_start_offset = 36;
        }
        else if (type == 21) {
          data.readFully(position + 8, buffer, 0, 32);
          resource_id = ByteArrayUtil.getLong(buffer, 4);
          page_number = ByteArrayUtil.getLong(buffer, 12);
          page_size   = ByteArrayUtil.getInt(buffer, 20);
          page_offset = ByteArrayUtil.getInt(buffer, 24);
          page_length = ByteArrayUtil.getInt(buffer, 28);
        ...
        // Read the content.
        data.readFully(position + data_start_offset,
   ...
   */
  private static abstract class AbstractResource implements JournalledResource {
    ...
    abstract void persistPageChange(final long page, final int page_size,
   ...
   */
  private static final class NonLoggingResource extends AbstractResource {

    /**
     * Constructs the resource.
     */
    NonLoggingResource(String name, long id,
                       StoreDataAccessor data, int page_size) {
      super(name, id, data, page_size);
    }
...
//      System.out.println("reading data " + page_number + " " + off);
...
//      System.out.println();
  ...
  private final class Resource extends AbstractResource {
     ...
     * Constructs the resource.
     */
    Resource(String name, long id, StoreDataAccessor data, int page_size) {
      super(name, id, data, page_size);
...
//        System.out.println("Building page from " + sz + " elements.");
        for (int i = 0; i < sz; ++i) {
