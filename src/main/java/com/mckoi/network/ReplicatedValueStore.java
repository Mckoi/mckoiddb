...
import com.mckoi.util.HexFormatter;
import java.io.*;
  ...
  public void debugOutput(PrintWriter out) {
    ...
    synchronized (block_db_write_lock) {
      ...
      try {
        ...
        // Create a hex formatter for debugging UID data,
        HexFormatter hex_formatter = new HexFormatter();
        hex_formatter.setByteLineWidth(16);
        ...
        for (long i = 0; i < size; ++i) {
          ...
          out.println(toUIDString(uid[0], uid[1]));
          byte[] buf = getValueFromUID(t, uid);
          if (buf == null) {
            out.println("(No value)");
          }
          else {
            hex_formatter.format(buf, out);
          }
          out.println();
